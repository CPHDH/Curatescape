<?php
function rssMainText($item = null){
...
function rssArticle($item, $url, $text=null){
	...
	$text .= rssMainText($item);
	$text .= rssReadMore($url, $stats);
	return $text;
}
...
foreach( loop( 'items' ) as $item )
{
	...
	$title = rssTitleCombo($item);
	$author = rssAuthors(dc( $item, 'Creator', array('all'=>true, 'no_filter'=>true)));
	$content = rssArticle($item, $url);
	// entry output
