...
class Curatescape_View_Helper_HookConfigForm extends Zend_View_Helper_Abstract{
	public function HookConfigForm(){
		...
		<fieldset>
			...
			<!-- Alt Item Type Name -->
			<?php echo $this->configFormText('curatescape_alt_item_type_name', __('Alternate Name'), __('Enter an alternate display name for the %1s Item Type. You may also need to adjust the corresponding link label in <a href="%2s" target="_blank">Navigation settings</a>.', _CURATESCAPE_ITEM_TYPE_NAME_, admin_url('appearance/edit-navigation')), 'Example: Story');?>
			<!-- Alt Item Type Name (Plural) -->
			<?php echo $this->configFormText('curatescape_alt_item_type_name_p', __('Alternate Name Plural'), __('Enter the plural form of your alternate display name for the %s Item Type. You may also need to adjust the corresponding link label in <a href="%2s" target="_blank">Navigation settings</a>.', _CURATESCAPE_ITEM_TYPE_NAME_, admin_url('appearance/edit-navigation')), 'Example: Stories');?>
		</fieldset>
		...
		<fieldset>
			...
			<?php echo $this->configFormCheckBox('curatescape_template', __('Metadata Template'), __("If checked, the Item metadata display will be modified to use an enhanced layout for Items using the %s Item Type. Other Item Types will fall back to the Omeka default template.", _CURATESCAPE_ITEM_TYPE_NAME_) );?>
			<!-- Link Select Elements -->
			<?php echo $this->configFormCheckBox('curatescape_metadata_browse', __('Link Select Elements'), __("If checked, the Creator and Subject metadata elements will automatically be converted into hyperlinks that point to related Items. Note that this option will be ignored if the Search By Metadata plugin is installed and activated.") );?>
			<!-- Auto Subtitle -->
			<?php echo $this->configFormCheckBox('curatescape_auto_subtitle', __('Append Subtitle'), __('If checked, the Subtitle will be automatically appended to the Title anywhere it is displayed, including the record title, &lt;meta&gt; title, and more.'));?>
			<!-- Subtitle Styles -->
			<?php echo $this->configFormCheckBox('curatescape_subtitle_styles', __('Subtitle Styles'), __('If checked, the Subtitle will be formatted using plugin styles that change the color, typography, and layout. If unchecked, the Subtitle will be separated from the title by a colon. Requires use of auto-appending subtitle option, selected above. May not be supported in some themes.'));?>
			<!-- Byline -->
			<?php echo $this->configFormSelect('curatescape_byline', __('Byline Location'), __('Select the location where the formatted byline should be displayed.'),
				array(
			...
			<!-- Inline Factoids -->
			<?php echo $this->configFormCheckBox('curatescape_inline_factoids', __('Inline Factoids'), __('If checked, Factoids will be displayed within the main text. If the main text is more than five paragraphs in length, Factoids will be displayed after the third paragraph. Factoids are always wrapped in an &lt;aside&gt; tag for optimal accessibility.'));?>
			<!-- Factoid Label -->
			<?php echo $this->configFormText('curatescape_factoids_label', __('Factoids Label'), __('Enter a custom label to replace the default heading for Factoids.'), 'Example: Did you know?');?>
		</fieldset>
		...
		<fieldset>
			...
			<!-- Alt Tour Name -->
			<?php echo $this->configFormText('curatescape_alt_tour_name', __('Alternate Name'), __('Enter an alternate display name for Tours. You may also need to adjust the corresponding link label in <a href="%s" target="_blank">Navigation settings</a>.', admin_url('appearance/edit-navigation')), 'Example: Tour');?>
			<!-- Alt Tour Name (Plural) -->
			<?php echo $this->configFormText('curatescape_alt_tour_name_p', __('Alternate Name Plural'), __('Enter the plural form of your alternate display name for Tours. You may also need to adjust the corresponding link label in <a href="%s" target="_blank">Navigation settings</a>.', admin_url('appearance/edit-navigation')), 'Example: Tours');?>
			<!-- Thumb Style-->
			<?php echo $this->configFormSelect('curatescape_tour_thumb_style', __('Tour Thumbnail'), __('Select the style of thumbnail image to use when browsing tours. The composite style combines the first image from the first four Items on the Tour to create a mini-collage.'),
				array(
		...
		<fieldset>
			...
			<span class="map-settings">
				...
				<!-- Map Primary -->
				<?php echo $this->configFormSelect('curatescape_map_primary_layer', __('Primary Map Style'), __('Select the primary map style.'),
					$mapLayers
				);?>
				<!-- Map Secondary -->
				<?php echo $this->configFormSelect('curatescape_map_secondary_layer', __('Secondary Map Style'), __('Select the secondary map style.'),
				...
				<span class="stadia-settings">
					<!-- Stadia Key -->
					<?php echo $this->configFormText('curatescape_map_stadia_key', __('Stadia Authentication'), __('Stadia Maps can be authenticated by registering this domain (%1s) in your <a target="_blank" href="%2s">Stadia account</a>. Alternately, you may enter your Stadia API key below. At least one method is required. Domain registration is the preferred method.', preg_replace('~^https?://~', '', WEB_ROOT), 'https://stadiamaps.com/'), 'Example: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx');?>
					<!-- Prefer EU -->
					<?php echo $this->configFormCheckBox('curatescape_map_prefer_eu', __('EU Preference'), __('If checked, load supported map tile sets from servers in the European Union.'));?>
				</span>
				<!-- Custom Label -->
				<?php echo $this->configFormText('curatescape_map_custom_label', __('Custom Map Label'), __('Enter custom labels for your map layers, separated by a comma. These labels will be displayed in the interface for switching between primary and secondary map layers (if enabled). The number of labels should match the number of map layers.'), 'Example: Street, Satellite');?>
				<!-- Marker Color -->
				<?php echo $this->configFormText('curatescape_map_marker_color', __('Marker Color'), __('Enter an HTML color code to use for map markers.'), 'Example: #222222');?>
				<!-- Featured Marker Color -->
				<?php echo $this->configFormText('curatescape_map_marker_featured_color', __('Featured Marker Color'), __('Enter an HTML color code to use for featured item map markers.'), 'Example: #222222');?>
				<!-- Featured Marker Star Icon -->
				<?php echo $this->configFormCheckBox('curatescape_map_marker_featured_star', __('Featured Marker Icon'), __('If checked, use a star icon inside featured item map markers.'));?>
				<!-- Clusters -->
				<?php echo $this->configFormCheckBox('curatescape_map_clusters', __('Clusters'), __('If checked, markers on the map will be grouped into clusters at higher zoom levels.'));?>
				<span class="cluster-settings">
					<!-- Cluster Colors -->
					<?php echo $this->configFormText('curatescape_map_cluster_colors', __('Cluster Colors'), __('Enter three valid CSS colors to customize the map clusters, each separated by a <code>|</code> pipe. The first color will be used for small clusters, the second for medium clusters, and the third for large clusters. Leave blank to use default colors.'), 'Example: teal | #008080 | rgb(0, 128, 128)');?>
				</span>
				<!-- Subjects Select -->
				<?php echo $this->configFormCheckBox('curatescape_map_subjects_select', __('Subjects Select'), __('If checked, allow users to select from a dropdown of Subject terms to filter items on the global map. Note that this option is only recommended when all items have been given at least one Subject term. Use of the <a target="_blank" href="%s">Simple Vocab plugin</a> is strongly recommended.', 'https://omeka.org/classic/plugins/SimpleVocab/'));?>
				<!-- Fixed Center-->
				<?php echo $this->configFormCheckBox('curatescape_map_fixed_center', __('Fixed Center'), __('If checked, the initial center of your multi-marker maps will always adhere to the <a target="_blank" href="%s">Geolocation plugin settings</a> for default latitude, longitude, and zoom level. This option does not apply to tours, to single items, or to maps where the user has explicitly chosen a specific set of parameters (such as a subject term). This option may be useful if your map contains outliers beyond the expected bounds of your coverage area.', '/admin/plugins/config?name=Geolocation'));?>
			</span>
		</fieldset>
		...
		<fieldset>
			...
			<!-- Admin Bar Edit-->
			<?php echo $this->configFormCheckBox('curatescape_admin_bar_edit', __('Admin Bar'), __('If checked, an "Edit" link will be appended to the default Omeka admin bar for supported record types. Theme support varies. Note that this option will be selectively ignored if the Admin Tools plugin is active and configured to add its own edit link for a given record type.'));?>
			<!-- Secondary Nav -->
			<?php echo $this->configFormCheckBox('curatescape_append_secondary_nav', __('Append Secondary'), __('If checked, new navigation items for Featured and Curatescape Stories will be added to the secondary/Item navigation for the items/browse view.'));?>
			<!-- Secondary Nav Shorten -->
			<?php echo $this->configFormCheckBox('curatescape_shorten_secondary_nav', __('Shorten Secondary'), __('If checked, the secondary (item) navigation labels will be shortened to preserve space in some themes. For example, "Browse By Tags" will be shortened to "Tags."'));?>
		</fieldset>
		...
		<fieldset>
			...
			<!-- Enhanced RSS-->
			<?php echo $this->configFormCheckBox('curatescape_rss', __('Enhanced RSS'), __('If checked, the default RSS feed will be replaced with an <a href="%s" target="_blank">enhanced format</a> that has been customized for narrative content.', items_output_url('rss-plus')));?>
		</fieldset>
		...
		<fieldset>
			...
			<!-- File Stats -->
			<?php echo $this->configFormCheckBox('curatescape_dashboard_stats', __('File Statistics'), __('If checked, the admin dashboard will display file statistics and format recommendations. File statistics are cached on the server side for up to 1 week or until an item record is added or modified. This option may temporarily slow down loading of the dashboard when the cache is being refreshed.', _PLUGIN_NAME_) );?>
			<!-- Project Admin -->
			<?php echo $this->configFormCheckBox('curatescape_dashboard_project_mgmt', __('Project Management'), __('If checked, the admin dashboard will display convenient links to manage configured app store and web analytics accounts. See App Store Settings below. Website analytics are derived from theme settings (option name: <code>google_analytics</code>) and supported third-party plugins (e.g. Matomo).') );?>
			<!-- Resources -->
			<?php echo $this->configFormCheckBox('curatescape_dashboard_resources', __('Resources'), __('If checked, the admin dashboard will display a list of useful %s resources.', _PLUGIN_NAME_) );?>
		...
		<fieldset>
			...
			<!-- iOS-->
			<?php echo $this->configFormText('curatescape_app_ios', __('iOS App'), __('Enter the numeric identifier for the iOS app, found in Apple App Store Connect account. When this option is enabled, an iOS Smart App Banner will be added to your site, and you will be able to use the %s shortcode to display iOS App Store links. Some themes may also use this option to systematically display iOS App Store links.', '<code>[curatescape_app_buttons]</code>'), 'Example: 0123456789');?>
			<!-- Android-->
			<?php echo $this->configFormText('curatescape_app_android', __('Android App'), __('Enter the alphanumeric package name/identifier for the Android app, found in Google Play Console account. When this option is enabled, you will be able to use the %s shortcode to display Google Play app links. Some themes may also use this option to systematically display Google Play app links.', '<code>[curatescape_app_buttons]</code>'), 'Example: com.developer.appname');?>
			<!-- Smart Banner -->
			<?php echo $this->configFormCheckBox('curatescape_smart_banner', __('Safari App Banner'), __('If checked, display a banner encouraging Safari users to download the configured iOS mobile app. (Android does not offer an equivalent functionality.)') );?>
		</fieldset>
		...
		<fieldset>
			...
			<!-- Plugin Styles -->
			<?php echo $this->configFormCheckBox('curatescape_plugin_styles', __('Plugin Styles'), __('If checked, use CSS styles provided by the %1s plugin. Turning this off may be useful for theme developers, but should otherwise remain checked.', _PLUGIN_NAME_) );?>
			<!-- Theme Fixes -->
			<?php echo $this->configFormCheckBox('curatescape_theme_fixes', __('Theme Fixes'), __('If checked, use theme-specific CSS styles provided by the %1s plugin. These styles apply to select themes and only affect elements related to the %2s plugin.', _PLUGIN_NAME_, _PLUGIN_NAME_));?>
			<!-- Omit Redundant -->
			<?php echo $this->configFormCheckBox('curatescape_omit_redundant_elements', __('Omit Redundant'), __('If checked, the Title and Subtitle elements for the Item will be omitted from the metadata table since they already appear as part of the page heading. Likewise, the Coverage element will be omitted since it already appears as part of the map caption (which consists of the Street Address and Access Information texts). Theme authors may apply their own filter to omit additional terms.', _CURATESCAPE_ITEM_TYPE_NAME_) );?>
		</fieldset>
